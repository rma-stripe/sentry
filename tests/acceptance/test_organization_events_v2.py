...
class OrganizationEventsV2Test(AcceptanceTestCase, SnubaTestCase):
    ...
    def test_transaction_event_detail_view_ops_filtering(self, mock_now):
        ...
        with self.feature(FEATURE_NAMES):
            ...
            # select all ops
            self.browser.elements('[data-test-id="op-filter-dropdown"] [data-test-id="checkbox-fancy"]')[0].click()

            # un-select django.middleware
            self.browser.elements('[data-test-id="op-filter-dropdown"] [data-test-id="checkbox-fancy"]')[1].click()
